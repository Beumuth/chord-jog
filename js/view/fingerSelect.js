var FingerSelect = (function () {
	...
	return {
		...
		create: (options={}) => {
			...
			//Event handlers
			// mouse move
			fingerSelect.addEventListener("mousemove", (e) => {
				//Get the preview region based on closest point to mouse
				const previewFinger = closestFingerToPoint(
					[e.offsetX, e.offsetY]);
			...
			//	mouse down
			fingerSelect.addEventListener("mousedown", (e) =>{
				if(e.button === 0)	//If left click,
					fingerSelect.selected =	//then select
						closestFingerToPoint(	//the closest point
							[e.offsetX, e.offsetY])	//to the cursor
			});
			//	mouse leave
			fingerSelect.addEventListener("mouseleave",
			...
			new MutationObserver((mutations) =>
				mutations.map(mutation => {
					...
					fingerLabelOutline.setAttribute("stroke",
						["preview", "selected"].includes(e.new) ?
							"black" : "none");
